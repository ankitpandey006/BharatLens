...
export default function SiteHeader() {
  ...
  const isAuthenticatedSurface = authenticatedRoutePrefixes.some((prefix) =>
    pathname.startsWith(prefix)
  );

  const isAuthenticated = Boolean(user) || isAuthenticatedSurface;
              ...
              <div ref={profileDropdownRef} className="group relative ml-2">
                ...
                >
                  ...
                  <button
                    ...
                    className="group/item flex w-full items-center gap-3 rounded-xl px-4 py-2.5 text-left text-sm font-medium text-red-600 transition-all duration-300 ease-out hover:bg-red-50"
                  >
                    <LogOut
                      size={16}
                      className="transition-transform duration-300 ease-out group-hover/item:scale-110"
          ...
          <div className="grid gap-2">
            {(isAuthenticated
              ...
              : [
                  ...
                  { href: "/chatbot", label: "Ask BharatLens AI", icon: Bot },
            ...
            {isAuthenticated ? (
              <button
                type="button"
                onClick={handleLogout}
                className="group flex w-full items-center gap-3 rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-600 transition-all duration-300 ease-out hover:bg-red-100"
              >
                <LogOut
                  size={18}
                  className="transition-transform duration-300 ease-out group-hover:scale-110"
