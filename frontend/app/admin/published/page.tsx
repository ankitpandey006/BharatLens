...
import { fetchAdminCollectedData, unpublishCollectedData, deleteCollectedData, editCollectedData, publishCollectedData, bulkUnpublishCollectedData, bulkDeleteCollectedData, type BulkActionResult } from "@/lib/api/admin";
...
export default function PublishedPage() {
  ...
  const refresh = useCallback(async () => {
    if (hasLoadedOnce.current) {
      ...
      try {
        const res = await fetchAdminCollectedData(1, 100, "published");
        setItems(res.items || []);
      } catch (err) {
  ...
  const handleBulkUnpublish = async () => {
    ...
    try {
      const result = await bulkUnpublishCollectedData(Array.from(selectedIds));
      setBulkResult(result);
      clearSelection();
      await refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Bulk unpublish failed");
    } finally {
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Delete ${selectedIds.size} published items permanently? These items are visible to users.`)) return;
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Delete this published item permanently?")) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1A3C6E] sm:text-3xl">Published Items</h1>
          <p className="mt-1 text-sm text-[#111827]/60">Items currently published and visible to users.</p>
        </div>
      ...
      <BulkActionBar
        ...
        result={bulkResult}
        pageLabel="items published"
        actions={[
          { label: "Unpublish", action: handleBulkUnpublish, variant: "warning" },
      ...
      <ModerationTable
        ...
        onUnpublish={(it) => handleUnpublish(it.id)}
        onDelete={(it) => handleDelete(it.id)}
        showActions={["preview", "edit", "unpublish", "delete"]}
