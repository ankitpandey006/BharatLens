...
import { Search, X, GraduationCap, BarChart3, Filter, ChevronDown, SlidersHorizontal, MapPin, BookOpen, Clock, FileText, Building2, Tag, Calendar, ShieldCheck } from "lucide-react";
import { getCategoryCounts, normalizeSubType } from "@/utils/getCategoryCounts";

type ExamTab = "all" | "apply_now" | "admit_card" | "result" | "answer_key" | "notification";
...
const TAB_EMPTY_STATES: Record<string, { title: string; icon: typeof GraduationCap }> = {
  all: { title: "No exams found", icon: GraduationCap },
  ...
  answer_key: { title: "No answer keys available", icon: BookOpen },
...
export default function ExamsPage() {
  ...
  const queryParams = useMemo(() => {
    ...
    if (sortBy === "deadline") { params.sortBy = "application_end_date"; params.sortOrder = "asc"; }
  ...
  const { data: result, error, isLoading } = useExams(queryParams);
  ...
  const tabCounts = useMemo(() => {
    return getCategoryCounts(exams, TABS.map((t) => t.key));
  }, [exams]);
  ...
  const uniqueCategories = useMemo(() => {
    ...
    result.items.forEach((e) => { if (e.category) cats.add(e.category); });
  ...
  const uniqueStates = useMemo(() => {
    ...
    result.items.forEach((e) => { if (e.state) sts.add(e.state); });
    return Array.from(sts).sort();
  }, [result?.items]);

  const hasFilters = debouncedSearch || activeTab !== "all" || sortBy !== "newest" || categoryFilter || stateFilter;
  const displayExams = activeTab === "all" ? exams : exams.filter((e) => normalizeSubType(e.sub_category) === activeTab);
  const EmptyIcon = TAB_EMPTY_STATES[activeTab]?.icon || GraduationCap;

  const toggleSaved = async (examId: string) => {
    setSavingMap((prev) => ({ ...prev, [examId]: true }));
    try {
      const currentlySaved = Boolean(savedMap[examId]);
      if (currentlySaved) await unsaveItem(examId, "exam");
      else await saveItem(examId, "exam");
      mutateSaved();
          ...
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                ...
                <h1 className="mt-1.5 text-2xl font-extrabold text-white sm:text-3xl lg:text-4xl">Government Exams</h1>
                <p className="mt-1.5 max-w-2xl text-sm leading-5 text-[#C7D9FE] sm:text-base">
                  Track notifications, applications, admit cards, answer keys &amp; results
                </p>
              </div>
              {/* Stats row inline */}
                  ...
                  {activeTab === "all" && (
                    <>
                      {[
                        ...
                        { key: "answer_key", label: "Key", color: "bg-white/15 text-white" },
                        { key: "notification", label: "Notify", color: "bg-white/15 text-white" },
                      ].map((stat) => {
                        const count = exams.filter((e) => normalizeSubType(e.sub_category) === stat.key).length;
                        return count > 0 ? (
      ...
      <div className="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:p-4">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search exams by title, conducting body, category..."
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-red-200 bg-red-50 p-10 text-center">
            ...
            <p className="mt-4 text-lg font-semibold text-red-700">Failed to load exams</p>
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-12 sm:p-16 text-center shadow-sm">
            ...
            <h3 className="mt-5 text-xl font-bold text-[#1A3C6E]">
              {TAB_EMPTY_STATES[activeTab]?.title || "No exams found"}
            </h3>
            <p className="mt-2 text-sm text-gray-500 max-w-sm mx-auto">
              {hasFilters
                ? "We couldn't find any exams matching your current filters. Try adjusting your search terms or clearing filters."
                : "Exam listings will appear here once they are published and verified."}
            </p>
            {hasFilters && (
              <button
          ...
          <>
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {displayExams.map((exam) => {
                const sc = normalizeSubType(exam.sub_category);
                const deadlineDate = exam.applicationDeadline ? new Date(exam.applicationDeadline) : null;
                ...
                const badgeConfig: Record<string, { label: string; gradient: string; dot: string }> = {
                  ...
                  answer_key: { label: "Answer Key", gradient: "from-[#1A3C6E] to-[#2D5A9E]", dot: "bg-blue-400" },
                ...
                const actionConfig: Record<string, { label: string; gradient: string }> = {
                  ...
                  answer_key: { label: "Download Answer Key", gradient: "from-[#1A3C6E] to-[#2D5A9E] hover:from-[#2D5A9E] hover:to-[#3B82F6]" },
                ...
                const metadata = [
                  { icon: <Building2 className="h-4 w-4" />, text: <><span className="font-medium text-gray-900">Conducting Body:</span> {exam.conductingBody || "Official Authority"}</> },
                  { icon: <Tag className="h-4 w-4" />, text: <><span className="font-medium text-gray-900">Category:</span> {exam.category || "General"}</> },
                ];
                if (exam.state) metadata.push({ icon: <MapPin className="h-4 w-4" />, text: <><span className="font-medium text-gray-900">State:</span> {exam.state}</> });
                if (exam.examDate) metadata.push({ icon: <Calendar className="h-4 w-4" />, text: <><span className="font-medium text-gray-900">Exam Date:</span> {exam.examDate}</> });
                if (exam.eligibility) metadata.push({ icon: <ShieldCheck className="h-4 w-4" />, text: <><span className="font-medium text-gray-900">Eligibility:</span> {exam.eligibility}</> });

                return (
                  <BharatLensCard
                    key={exam.id}
                    detailHref={`/exams/${exam.id}`}
                    badge={{ label: bc.label, gradient: bc.gradient, dotColor: bc.dot }}
                    isOfficial={exam.verification_status === "published"}
                    title={exam.title || "Untitled"}
                    description={exam.description || undefined}
                    metadata={metadata}
                    deadline={exam.applicationDeadline || undefined}
                    isExpiringSoon={isExpiringSoon}
                    sourceUrl={exam.source_url || undefined}
                    createdAt={exam.created_at || undefined}
                    primaryAction={{
                      label: ac.label,
                      href: exam.official_url || exam.apply_url || undefined,
