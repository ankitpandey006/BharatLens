      ...
      <div className="w-full max-w-full px-4 py-4 sm:px-6 sm:py-6 lg:mx-auto lg:max-w-7xl lg:px-8">
        ...
        {!profile.completed && profile.missingFields.length > 0 && (
          <section className="mt-4 w-full max-w-full overflow-hidden rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <div className="flex w-full max-w-full items-start gap-3">
              ...
              <Link href="/profile/setup" className="shrink-0 rounded-full bg-amber-600 px-3 py-2 text-xs font-semibold text-white">
                Complete
              </Link>
            </div>
          </section>
        ...
        <section className="mt-4 grid w-full max-w-full grid-cols-2 gap-3 sm:mt-6 sm:gap-4 lg:grid-cols-4">
          {statCards.map((item) => {
            ...
            return (
              <Link key={item.label} href={item.href} prefetch={false} className="group w-full overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm transition hover:shadow-md sm:p-5">
                ...
                <p className="mt-1 overflow-hidden text-ellipsis whitespace-nowrap text-xs font-medium text-[#111827]/60">{item.label}</p>
              </Link>
            );
          })}
        </section>
        ...
        <section className="mt-4 grid w-full max-w-full gap-4 sm:mt-6 lg:grid-cols-[1fr_320px] xl:grid-cols-[1fr_360px]">
            ...
            <div className="w-full max-w-full overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm sm:p-6">
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2 overflow-hidden">
                  ...
                  <h2 className="truncate text-base font-bold text-[#111827] sm:text-lg">AI Recommendations</h2>
                </div>
                {hasRecommendations && (
                  <Link href="/recommendations" className="shrink-0 text-xs font-semibold text-[#3B82F6]">View all</Link>
                )}
              </div>
              <div className="mt-4 space-y-3">
                {hasRecommendations ? (
                  recommendations.slice(0, 3).map((rec) => (
                    <div key={rec.id} className="w-full overflow-hidden rounded-xl border border-[#E5E7EB] bg-[#F5F3EE] p-4">
                      <div className="flex w-full max-w-full items-start justify-between gap-3">
                        <div className="min-w-0 flex-1 overflow-hidden">
                          <span className="inline-block max-w-full truncate rounded-full bg-white px-2.5 py-0.5 text-[10px] font-bold text-[#1A3C6E]">{rec.tag}</span>
                          <p className="mt-2 line-clamp-2 wrap-break-words text-sm font-semibold leading-snug text-[#111827]">{rec.title}</p>
                        </div>
                  ...
                  <EmptyState icon={Zap} title="No recommendations yet" description={profile.completed ? "Check back later for personalized matches." : "Complete your profile to get AI-powered recommendations."} action={!profile.completed ? { label: "Complete Profile", href: "/profile/setup" } : undefined} />
                )}
              </div>
            </div>
              ...
              <div className="mt-4 space-y-2">
                {hasUpdates ? (
                  todayUpdates.map((update, i) => (
                    <div key={`${update.type}-${i}`} className="flex w-full max-w-full items-start gap-3 overflow-hidden rounded-xl border border-[#E5E7EB] p-3">
                      <span className="shrink-0 rounded-lg bg-[#1A3C6E]/10 px-2 py-1 text-[10px] font-bold text-[#1A3C6E]">{update.type}</span>
                      <div className="min-w-0 flex-1 overflow-hidden">
                  ...
                  <EmptyState icon={CalendarDays} title="No recent updates" description="New opportunities will appear here as they are added." />
                )}
              </div>
            </div>
          </div>
          ...
          <aside className="grid w-full max-w-full gap-4 lg:block lg:space-y-5">
            <div className="w-full max-w-full overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm sm:p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                {counts.notifications > 3 && (
                  <span className="shrink-0 rounded-full bg-[#3B82F6]/10 px-2 py-0.5 text-[10px] font-bold text-[#3B82F6]">{counts.notifications}</span>
                )}
              </div>
              <div className="mt-4 space-y-2">
                {hasNotifications ? (
                  notificationsList.slice(0, 3).map((n) => (
                    <div key={n.id} className={`w-full max-w-full overflow-hidden wrap-break-words rounded-xl p-3 text-sm leading-5 ${n.is_read ? "bg-white text-[#111827]/60" : "bg-[#F5F3EE] text-[#111827]/80"}`}>{n.message}</div>
                  ))
                ) : (
                  <EmptyState icon={Bell} title="No notifications" description="Updates about your opportunities will appear here." />
                )}
              </div>
            </div>
            ...
            <div className="w-full max-w-full overflow-hidden rounded-2xl border border-[#E5E7EB] bg-[#1A3C6E] p-4 text-white shadow-lg shadow-[#1A3C6E]/15 sm:p-5">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/15"><Bot size={22} /></div>
              <h2 className="mt-4 wrap-break-words text-base font-bold sm:text-lg">Ask BharatLens AI</h2>
              <p className="mt-2 wrap-break-words text-xs leading-6 text-white/70">Ask about eligibility, documents, deadlines, or benefits for any opportunity.</p>
              <Link href="/chatbot" className="mt-4 flex w-full max-w-full items-center justify-center gap-1 rounded-full bg-white px-4 py-2 text-xs font-semibold text-[#1A3C6E]">Open assistant <ArrowRight size={12} /></Link>
            </div>
            ...
            <div className="w-full max-w-full overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm sm:p-5">
              ...
              {counts.savedItems > 0 ? (
                <>
                  <p className="mt-3 wrap-break-words text-sm leading-6 text-[#111827]/60">You have <strong className="text-[#1A3C6E]">{counts.savedItems}</strong> saved {counts.savedItems === 1 ? "item" : "items"} ready for review.</p>
                  <Link href="/saved" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[#3B82F6]">View saved items <ArrowRight size={12} /></Link>
                </>
              ) : (
                <EmptyState icon={Bookmark} title="No saved items" description="Save schemes, scholarships, jobs, or exams to review later." action={{ label: "Browse opportunities", href: "/schemes" }} />
              )}
            </div>
          </aside>
        </section>
