...
export default function NotificationsPage() {
  ...
  const { data: result, error, isLoading } = useNotifications({ page, limit: 20 });
        ...
        <div className="rounded-2xl border border-[#E5E7EB] bg-white p-5 shadow-md sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#111827]/75">Stay updated with reminders, matches, updates, and alerts from your saved items.</p>
            </div>
            <span className="rounded-full bg-[#EEF2FF] px-4 py-2 text-sm font-semibold text-[#1A3C6E]">{unreadCount} unread</span>
          </div>
        </div>

        {isLoading ? (
          <div className="mt-8"><ListSkeleton count={5} compact /></div>
        ) : error ? (
          <div className="mt-8 rounded-2xl border border-[#E5E7EB] bg-red-50 p-8 text-center shadow-md">
            <p className="text-lg font-semibold text-red-600">{error instanceof Error ? error.message : "Failed to load notifications"}</p>
          </div>
              ...
              {notifications.map((notification) => (
                <NotificationCard
                  ...
                  notification={{
                    ...
                    createdAt: notification.created_at ? new Date(notification.created_at).toLocaleDateString() : new Date().toLocaleDateString(),
